...
export const dmpTrackerColDefs: Array<ColGroupDef<DmpTrackerRecord>> = [
  {
    headerName: "DmpSampleTrackerData",
    groupId: "Group1",
    children: [
      {
        field: "request_reference_number",
        headerName: "iLabRequest ID",
        pinned: true,
      },
      { field: "dmp_sample_id", headerName: "DMPSampleID", pinned: true },
      ...
      {
        field: "sample_status",
        headerName: "Sample status for WES samples only",
  ...
  },
  {
    ...
    children: [
      ...
      {
        field: "consentPartA",
        headerName: "Consent Part A",
      },
      {
        field: "consentPartC",
        headerName: "Consent Part C",
      },
    ],
  },
];

export function setupColDefConfigs(
  dmpTrackerColDefs: Array<ColGroupDef> | Array<ColDef>
) {
  dmpTrackerColDefs.forEach((colDef) => {
    if ("children" in colDef && colDef.children) {
      // If the colDef is a group, recursively set up its children
      for (const childColDef of colDef.children) {
        if (
          "field" in childColDef &&
          typeof childColDef.field === "string" &&
          columnGroupOpen.includes(childColDef.field)
        ) {
          childColDef.columnGroupShow = "open";
        }
      }
    }
  });
...
setupColDefConfigs(dmpTrackerColDefs);
